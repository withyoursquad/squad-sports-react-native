...
import { performCleanup as performImageCleanup } from './utils/ImageOptimizationManager';
...
// --- Internal: boot journey system (validate → recover → hydrate) ---

function JourneyBootSentinel({ apiClient }: { apiClient: SquadApiClient }) {
  useEffect(() => {
    let cancelled = false;
    ...
    const boot = async () => {
      try {
        ...
        const report = await validateStateIntegrity(storage);

        if (cancelled) return;

        // Step 2: Run recovery if needed
        if (report.status !== 'healthy') {
          const engine = new StateRecoveryEngine(storage);
          await engine.executeRecovery(report);
        }

        if (cancelled) return;

        // Step 3: Hydrate journey state from API
        const connector = new JourneyApiConnector(apiClient, storage);
        await connector.hydrate();
      } catch (err) {
        // Non-fatal — journey features degrade gracefully
        try {
          Logger.shared?.warn?.('[JourneyBoot] Boot failed:', err);
        } catch {
          ...
          <SafeAreaProvider>
            <GestureHandlerRootView style={{ flex: 1 }}>
              ...
              >
                ...
                <JourneyBootSentinel apiClient={sdk.apiClient} />
                {children}
              </ThemeProvider>
